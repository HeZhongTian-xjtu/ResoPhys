...
class MultiHeadedSelfAttention_TDC_gra_sharp(nn.Module):
    ...
    def forward(self, x, gra_sharp):    # [B, 4*4*40, 128]
        ...
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        ...
        q = q.flatten(2).transpose(1, 2)  # [B, 4*4*40, dim//2]
        k = k.flatten(2).transpose(1, 2)  # [B, 4*4*40, dim//2]
        v = v.flatten(2).transpose(1, 2)  # [B, 4*4*40, dim//2]
        
        q, k, v = (split_last(x, (self.n_heads, -1)).transpose(1, 2) for x in [q, k, v])
        # (B, H, S, W) @ (B, H, W, S) -> (B, H, S, S) -softmax-> (B, H, S, S)
        scores = q @ k.transpose(-2, -1) / gra_sharp

        scores = self.drop(F.softmax(scores, dim=-1))   # [B, H, S, S]
        # (B, H, S, S) @ (B, H, S, W) -> (B, H, S, W) -trans-> (B, S, H, W)
        h_global = (scores @ v).transpose(1, 2).contiguous()   
        # -merge-> (B, S, D)
        h_global = merge_last(h_global, 2)
        
        self.scores = scores
        
        return h_global, scores
