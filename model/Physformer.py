...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    ...
    def __init__(
        ...
        self.ConvBlockLast = nn.Conv1d(dim//4, 1, 1,stride=1, padding=0)
    ...
    def forward(self, inputs):
        x, gra_sharp = inputs['input_clip'], inputs['gra_sharp']
        ...
        features_last = Trans_features3.transpose(1, 2).view(b, self.dim, t//8, 8, 8) # [B, 64, 40, 4, 4]
        
        features_last = self.upsample(features_last)		    # x [B, 64, 7*7, 80]
        features_last = self.upsample2(features_last)		    # x [B, 32, 7*7, 160]
        features_last = self.upsample3(features_last)
