...
class SA_upsample(nn.Module):
    ...
    def forward(self, x, scale, scale2):
        ...
        # permute() can reorder dimensions
        ...
        embedding = self.body(input)  # the above encodes a coordinate
        # offsets
...
class SA_adapt(nn.Module):
    ...
    def forward(self, x, scale, scale2):
        ...
        adapted = self.adapt(x, scale, scale2)  # add a convolution layer that accounts for scaling factors
...
class PhysMamba_sda(nn.Module):
    def __init__(self, scale1,scale2,S=2, in_ch=3, theta=0.5, drop_rate1=0.25, drop_rate2=0.5, frames=160):
        super(PhysMamba_sda, self).__init__()
        ...
        self.InConvBlock1 = nn.Sequential(
            nn.Conv3d(in_ch, 16, [1, 3, 3], stride=1, padding=[0, 1, 1]),
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
        )

        self.InConvBlock1_1 = nn.Sequential(
            nn.Conv3d(in_ch, 16, [1, 3, 3], stride=1, padding=[0, 1, 1]),
            nn.BatchNorm3d(16),
            nn.ReLU(),
        )
        ...
        self.Mamba_ConvBlock1 = conv_block(16, 16, [1, 5, 5], stride=1, padding=[0, 2, 2])  
    ...
    def forward(self, x, y):
        ...
        parity = []
        x = self.InConvBlock1(x)  # (B, 16, T, 128, 128)
        w = x.size(3)
        self.scale1 = 128.0 / w
        # x = self.ConvBlock2(x)
        # x = self.ConvBlock3(x)
        y = self.InConvBlock1_1(y)  # (B, 16, T, W2, H2)
        w = y.size(3)
        self.scale2 = 128.0 / w
    ...
    # # Ablation experiment 3
        ...
        x = self.Mamba_upsample2(x_fusion)  # [B, 48, T, H, W]
...
def load_model(args, device):
    return PhysMamba_sda(0,0)

if __name__ == "__main__":
    model = PhysMamba_sda(0,0)
    model.cuda()
